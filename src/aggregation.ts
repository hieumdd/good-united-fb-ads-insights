...
import { createView } from './db/mongo';
import { models as model } from './facebook/facebookModel';
...
const aggregationPipelines = (schema: Schema) => {
    ...
    const project = {
        $project: {
            data: {
                $concatArrays: [...scalar, ...nested].map(
                    ({ path }) => `$${path}`,
                ),
            },
        },
        ...
        $group: {
            ...
            ads: {
                $push: {
                    date_start: '$date_start',
                    ...Object.fromEntries(
                        scalar.map(({ path }) => [path, `$${path}`]),
                    ),
                    ...Object.fromEntries(
                        nested.map(({ path }) => [path, `$${path}`]),
                    ),
                },
            },
        },
...
createView('FBAdsJoined', 'Events', aggregationPipelines(model.schema)).then(
    ([viewErr, view]) => {
        if (viewErr) {
            console.log(viewErr);
        } else {
